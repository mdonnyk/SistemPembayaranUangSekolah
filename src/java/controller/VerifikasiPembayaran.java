...
/**
 *
 * @author Alicia Destriani Sandea
 */
...
public class VerifikasiPembayaran extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String nis = request.getParameter("nis").replaceAll("\\s+","");
        String nama = request.getParameter("nama").replaceAll("\\s+","");
        String jenis = request.getParameter("jenis");
        String tanggal = request.getParameter("tanggal").replaceAll("\\s+","");
        String jumlahP = request.getParameter("jumlah").replaceAll("\\s+","");
        double jumlah = 0;
        ...
        for (int i = 0; i < p.length; i++) {
            if (
                    p[i].getNis().equalsIgnoreCase(nis.replaceAll("\\s+","")) // bandingkan nis tanpa whitespace dan uppercase
